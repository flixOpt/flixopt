...
class Storage(Component):
    ...
    **Storages** have one incoming and one outgoing **Flow** - $f_\text{in}$ and $f_\text{out}$ -
    each with an efficiency $\eta_\text{in}$ and $\eta_\text{out}$. Further, storages have a size $\text C$ and a
    state of charge $c(\text{t}_i)$.
    Similarly to the flow-rate $p(\text{t}_i)$ of a `Flow`, the `size` $\text C$ combined with a relative upper bound
