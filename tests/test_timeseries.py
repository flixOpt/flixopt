...
from flixopt.core import ConversionError, DataConverter, TimeSeriesCollection, TimeSeriesData
...
class TestTimeSeries:
    ...
    def test_initialization(self, simple_dataarray):
        ...
        assert ts.equals(simple_dataarray)
    ...
    def test_active_timesteps_getter_setter(self, sample_timeseries, sample_timesteps):
        ...
        assert sample_timeseries.equals(sample_timeseries.stored_data.sel(time=subset_index))
    ...
    def test_reset(self, sample_timeseries, sample_timesteps):
        ...
        assert sample_timeseries.active_timesteps.equals(sample_timesteps)
        assert sample_timeseries.equals(sample_timeseries.stored_data)
    ...
    def test_restore_data(self, sample_timeseries, simple_dataarray):
        ...
        assert sample_timeseries.equals(original_data)
    ...
    def test_arithmetic_operations(self, sample_timeseries):
        ...
        assert np.array_equal(
            (sample_timeseries + ts2).values, sample_timeseries.values + ts2.values
        )
        assert np.array_equal(
            (sample_timeseries - ts2).values, sample_timeseries.values - ts2.values
        )
        ...
        assert np.array_equal((sample_timeseries + data2).values, sample_timeseries.values + data2.values)
        assert np.array_equal((data2 + sample_timeseries).values, data2.values + sample_timeseries.values)

        # Test operations with scalars
        assert np.array_equal((sample_timeseries + 5).values, sample_timeseries.values + 5)
        assert np.array_equal((5 + sample_timeseries).values, 5 + sample_timeseries.values)

        # Test unary operations
        assert np.array_equal((-sample_timeseries).values, -sample_timeseries.values)
        assert np.array_equal((+sample_timeseries).values, +sample_timeseries.values)
        assert np.array_equal((abs(sample_timeseries)).values, abs(sample_timeseries.values))
    ...
    def test_numpy_ufunc(self, sample_timeseries):
        ...
        assert np.array_equal(np.add(sample_timeseries, 5).values, np.add(sample_timeseries, 5).values)

        assert np.array_equal(
            np.multiply(sample_timeseries, 2).values, np.multiply(sample_timeseries, 2).values
        )
        ...
        assert np.array_equal(
            np.add(sample_timeseries, ts2).values, np.add(sample_timeseries, ts2).values
        )
    ...
    def test_sel_and_isel_properties(self, sample_timeseries):
        ...
        assert selected.item() == sample_timeseries.values[0]

        # Test that isel property works
        indexed = sample_timeseries.isel(time=0)
        assert indexed.item() == sample_timeseries.values[0]
...
class TestTimeSeriesCollection:
    ...
    def test_create_time_series(self, sample_collection):
        ...
        assert np.all(ts1.values == 42)

        # Test numpy array
        data = np.array([1, 2, 3, 4, 5])
        ts2 = sample_collection.create_time_series(data, 'array_series')
        assert np.array_equal(ts2.values, data)
        ...
        assert len(ts4) == len(sample_collection.timesteps_extra)
    ...
    def test_insert_new_data(self, populated_collection, sample_timesteps):
        ...
        # Verify updates
        assert np.all(populated_collection['constant_series'].values == 100)
        assert np.array_equal(populated_collection['varying_series'].values, np.array([5, 10, 15, 20, 25]))

        # Series not in the DataFrame should be unchanged
        assert np.array_equal(
            populated_collection['extra_timestep_series'].values[:-1], np.array([1, 2, 3, 4, 5])
        )
    ...
    def test_restore_data(self, populated_collection):
        ...
        assert np.all(populated_collection['constant_series'].values == 999)
