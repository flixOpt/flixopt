...
### Shares to Effects
Effects are used to allocate things like costs, emissions, or other "effects" occuring in the system.
These arise from so called **Shares**, which originate from **Elements** like [Flows](#flows).
Flows for example can have `effects per flow hour` like costs, which would be given in [€/kWh] or emissions in [kg/kWh].
Effects are seperatly allocated for investments and operation.
...
Shares to the penalty can originate from every Element and are constructed similarly to $\eqref{Share_invest}$ and  $\eqref{Share_operation}$.
