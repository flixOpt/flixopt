...
class LinearConvertersModel:
    """Type-level model for batched conversion constraints across LinearConverters.

    Handles conversion factor constraints for ALL LinearConverters using mask-based
    batching. Each converter can have different numbers of conversion equations,
    handled through a padded coefficient structure.

    Note:
        Piecewise conversions are excluded and handled per-element in LinearConverterModel
        due to their additional complexity (segment selection binaries, etc.).

    Pattern:
        - coefficient array: (converter, equation_idx, flow) - conversion coefficients
        - sign array: (converter, flow) - +1 for inputs, -1 for outputs
        - equation_mask: (converter, equation_idx) - which equations exist

    Constraint: For each equation i in each converter c:
    ...
    def __init__(
        ...
        """Initialize the type-level model for LinearConverters.

        Args:
            model: The FlowSystemModel to create constraints in.
            converters: List of LinearConverters with conversion_factors (not piecewise).
        ...
        self._logger.debug(f'LinearConvertersModel initialized: {len(converters)} converters')
    ...
    def _max_equations(self) -> int:
        ...
        if not self.converters:
            return 0
        return max(len(c.conversion_factors) for c in self.converters)
    ...
    def create_constraints(self) -> None:
        """Create batched conversion factor constraints.
        ...
        for eq_idx in range(self._max_equations):
            ...
            converters_with_eq = [
                cid
                for cid, conv in zip(self.element_ids, self.converters, strict=False)
                if eq_idx < len(conv.conversion_factors)
            ]
