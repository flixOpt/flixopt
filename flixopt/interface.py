...
@dataclass
class StatusParametersBatched:
    """Batched status parameters with element dimension.

    This class concatenates status parameters from multiple elements into
    DataArrays for vectorized constraint creation. Uses NaN to indicate
    "no constraint for this element", enabling masking operations.

    Use this class instead of per-element StatusParameters when creating
    batched constraints in type-level models (StatusesModel).

    Attributes:
        element_ids: List of element identifiers (e.g., flow label_full).
        dim_name: Dimension name for element grouping (e.g., 'flow', 'component').
        active_hours_min: Minimum active hours per period. NaN = no constraint.
        active_hours_max: Maximum active hours per period. NaN = no constraint.
        min_uptime: Minimum consecutive uptime. NaN = no constraint.
        max_uptime: Maximum consecutive uptime. NaN = no constraint.
        min_downtime: Minimum consecutive downtime. NaN = no constraint.
        max_downtime: Maximum consecutive downtime. NaN = no constraint.
        startup_limit: Maximum startups per period. NaN = no constraint.
        effects_per_startup: Effect factors per startup, keyed by effect name.
        effects_per_active_hour: Effect factors per active hour, keyed by effect name.
        cluster_modes: Cluster mode per element ('relaxed' or 'cyclic').

    Example:
        >>> params = StatusParametersBatched.from_elements(
        ...     elements=flows_with_status,
        ...     parameters_getter=lambda f: f.status_parameters,
        ...     dim_name='flow',
        ... )
        >>> # Check which elements need startup tracking
        >>> mask = params.needs_startup_tracking
        >>> startup_elements = [eid for eid, m in zip(params.element_ids, mask.values) if m]
    """

    element_ids: list[str]
    dim_name: str

    # Bounds as DataArrays - NaN means "no constraint"
    active_hours_min: xr.DataArray
    active_hours_max: xr.DataArray
    min_uptime: xr.DataArray
    max_uptime: xr.DataArray
    min_downtime: xr.DataArray
    max_downtime: xr.DataArray
    startup_limit: xr.DataArray

    # Effects as DataArrays keyed by effect name - NaN means "no effect"
    effects_per_startup: dict[str, xr.DataArray] = field(default_factory=dict)
    effects_per_active_hour: dict[str, xr.DataArray] = field(default_factory=dict)

    # Per-element configuration
    cluster_modes: list[str] = field(default_factory=list)
    force_startup_tracking: xr.DataArray = field(default_factory=lambda: xr.DataArray([]))
    ...
    def from_elements(
        ...
        """Build batched parameters from list of elements with status_parameters.
        ...
        Args:
            ...
            dim_name: Dimension name for element grouping (e.g., 'flow').
            label_getter: Optional function to get element label. Defaults to e.label_full.
                e.g., lambda c: c.label (for components)

        Returns:
            StatusParametersBatched with concatenated parameters.
        """
        ...
        element_ids = [label_getter(e) for e in elements]

        def collect_param(attr: str) -> xr.DataArray:
            """Collect a parameter from all elements, converting None to NaN."""
            values = []
            for elem in elements:
                params = parameters_getter(elem)
            ...
            if all(isinstance(v, (int, float)) or (isinstance(v, float) and np.isnan(v)) for v in values):
                return xr.DataArray(values, dims=[dim_name], coords={dim_name: element_ids})
            else:
                # Convert scalars to DataArrays and concatenate
                expanded = []
                for val, eid in zip(values, element_ids, strict=False):
                    if isinstance(val, xr.DataArray):
                        arr = val.expand_dims({dim_name: [eid]})
                    else:
                        arr = xr.DataArray([val], dims=[dim_name], coords={dim_name: [eid]})
                    expanded.append(arr)
                return xr.concat(expanded, dim=dim_name, coords='minimal')

        def collect_effects(attr: str) -> dict[str, xr.DataArray]:
            """Collect effect dicts into DataArrays per effect."""
            # Find all effect names across all elements
            all_effects: set[str] = set()
            for elem in elements:
                params = parameters_getter(elem)
                effects = getattr(params, attr) or {}
                ...
                if all(isinstance(v, (int, float)) or (isinstance(v, float) and np.isnan(v)) for v in values):
                    ...
                    for val, eid in zip(values, element_ids, strict=False):
                        if isinstance(val, xr.DataArray):
                            ...
                            arr = xr.DataArray([val], dims=[dim_name], coords={dim_name: [eid]})
                        expanded.append(arr)
                    result[effect_name] = xr.concat(expanded, dim=dim_name, coords='minimal')
            return result

        # Collect cluster modes and force_startup_tracking
        cluster_modes = [parameters_getter(e).cluster_mode for e in elements]
        force_flags = [parameters_getter(e).force_startup_tracking for e in elements]
        force_startup_tracking = xr.DataArray(force_flags, dims=[dim_name], coords={dim_name: element_ids})

        return cls(
            element_ids=element_ids,
            dim_name=dim_name,
            active_hours_min=collect_param('active_hours_min'),
            active_hours_max=collect_param('active_hours_max'),
            min_uptime=collect_param('min_uptime'),
            max_uptime=collect_param('max_uptime'),
            min_downtime=collect_param('min_downtime'),
            max_downtime=collect_param('max_downtime'),
            startup_limit=collect_param('startup_limit'),
            effects_per_startup=collect_effects('effects_per_startup'),
            effects_per_active_hour=collect_effects('effects_per_active_hour'),
            cluster_modes=cluster_modes,
            force_startup_tracking=force_startup_tracking,
        )

    # === Convenience masks ===

    @property
    def needs_startup_tracking(self) -> xr.DataArray:
        """Boolean mask: elements needing startup/shutdown variables.

        True if element has effects_per_startup, startup_limit, uptime tracking,
        or force_startup_tracking is set.
        """
        has_startup_effects = xr.DataArray(
            [bool(self.effects_per_startup) for _ in self.element_ids],
            dims=[self.dim_name],
            coords={self.dim_name: self.element_ids},
        )
        has_startup_limit = self.startup_limit.notnull()
        has_uptime = self.min_uptime.notnull() | self.max_uptime.notnull()
        return has_startup_effects | has_startup_limit | has_uptime | self.force_startup_tracking

    @property
    def needs_uptime_tracking(self) -> xr.DataArray:
        """Boolean mask: elements with min_uptime or max_uptime."""
        return self.min_uptime.notnull() | self.max_uptime.notnull()

    @property
    def needs_downtime_tracking(self) -> xr.DataArray:
        """Boolean mask: elements with min_downtime or max_downtime."""
        return self.min_downtime.notnull() | self.max_downtime.notnull()

    @property
    def needs_startup_limit(self) -> xr.DataArray:
        """Boolean mask: elements with startup_limit."""
        return self.startup_limit.notnull()

    def get_elements_for_mask(self, mask: xr.DataArray) -> list[str]:
        """Get element IDs where mask is True."""
        return [eid for eid, m in zip(self.element_ids, mask.values, strict=False) if m]


@dataclass
class InvestParametersBatched:
    """Batched investment parameters with element dimension.

    This class concatenates investment parameters from multiple elements into
    DataArrays for vectorized constraint creation. Uses NaN to indicate
    "no constraint for this element", enabling masking operations.

    Use this class instead of per-element InvestParameters when creating
    batched constraints in type-level models (InvestmentsModel).

    Attributes:
        element_ids: List of element identifiers.
        dim_name: Dimension name for element grouping.
        minimum_size: Minimum investment size. NaN = use default epsilon.
        maximum_size: Maximum investment size. Required.
        fixed_size: Fixed size (creates binary decision). NaN = continuous sizing.
        mandatory: Boolean mask - True if investment is required.
        linked_periods: Binary mask for period linking. NaN = no linking.
        effects_of_investment: Fixed effects per investment.
        effects_of_investment_per_size: Effects proportional to size.
        effects_of_retirement: Effects if not investing.

    Example:
        >>> params = InvestParametersBatched.from_elements(
        ...     elements=flows_with_investment,
        ...     parameters_getter=lambda f: f.size,
        ...     dim_name='flow',
        ... )
        >>> # Check which elements are mandatory
        >>> mandatory_ids = [eid for eid, m in zip(params.element_ids, params.mandatory.values) if m]
    """

    element_ids: list[str]
    dim_name: str

    # Size bounds as DataArrays
    minimum_size: xr.DataArray
    maximum_size: xr.DataArray
    fixed_size: xr.DataArray  # NaN means continuous sizing

    # Boolean mask for mandatory investments
    mandatory: xr.DataArray

    # Period linking (binary DataArray or None)
    linked_periods: xr.DataArray | None = None

    # Effects as DataArrays keyed by effect name
    effects_of_investment: dict[str, xr.DataArray] = field(default_factory=dict)
    effects_of_investment_per_size: dict[str, xr.DataArray] = field(default_factory=dict)
    effects_of_retirement: dict[str, xr.DataArray] = field(default_factory=dict)
    ...
    def from_elements(
        ...
        """Build batched parameters from list of elements with InvestParameters.

        Args:
            elements: List of elements with InvestParameters.
            parameters_getter: Function to get InvestParameters from element.
                e.g., lambda f: f.size (for flows)
            dim_name: Dimension name for element grouping.
            label_getter: Optional function to get element label. Defaults to e.label_full.

        Returns:
            InvestParametersBatched with concatenated parameters.
        """
        if label_getter is None:
        ...
        def collect_param(attr: str, default=np.nan) -> xr.DataArray:
            """Collect a parameter from all elements, converting None to default."""
            values = []
            for elem in elements:
                params = parameters_getter(elem)
                val = getattr(params, attr)
            ...
            # Handle mixed scalar/DataArray values
            if all(not isinstance(v, xr.DataArray) for v in values):
                return xr.DataArray(values, dims=[dim_name], coords={dim_name: element_ids})
            else:
                expanded = []
                for val, eid in zip(values, element_ids, strict=False):
        ...
        # Collect mandatory flags
        mandatory_flags = [parameters_getter(e).mandatory for e in elements]
        mandatory = xr.DataArray(mandatory_flags, dims=[dim_name], coords={dim_name: element_ids})

        # Collect linked_periods - more complex due to varying period dims
        linked_periods_list = [parameters_getter(e).linked_periods for e in elements]
        if all(lp is None for lp in linked_periods_list):
            linked_periods = None
        else:
            # Handle mixed None/DataArray
            expanded = []
            for lp, eid in zip(linked_periods_list, element_ids, strict=False):
                if lp is None:
                    # Use scalar NaN for elements without linked_periods
                    arr = xr.DataArray([np.nan], dims=[dim_name], coords={dim_name: [eid]})
                elif isinstance(lp, xr.DataArray):
                    arr = lp.expand_dims({dim_name: [eid]})
                else:
                    arr = xr.DataArray([lp], dims=[dim_name], coords={dim_name: [eid]})
                expanded.append(arr)
            linked_periods = xr.concat(expanded, dim=dim_name, coords='minimal')

        return cls(
            element_ids=element_ids,
            dim_name=dim_name,
            minimum_size=collect_param('minimum_size', default=CONFIG.Modeling.epsilon),
            maximum_size=collect_param('maximum_size'),
            fixed_size=collect_param('fixed_size'),
            mandatory=mandatory,
            linked_periods=linked_periods,
            effects_of_investment=collect_effects('effects_of_investment'),
            effects_of_investment_per_size=collect_effects('effects_of_investment_per_size'),
            effects_of_retirement=collect_effects('effects_of_retirement'),
        )

    # === Convenience properties ===

    @property
    def minimum_or_fixed_size(self) -> xr.DataArray:
        """Returns fixed_size where set, otherwise minimum_size."""
        return xr.where(self.fixed_size.notnull(), self.fixed_size, self.minimum_size)

    @property
    def maximum_or_fixed_size(self) -> xr.DataArray:
        """Returns fixed_size where set, otherwise maximum_size."""
        return xr.where(self.fixed_size.notnull(), self.fixed_size, self.maximum_size)

    @property
    def is_non_mandatory(self) -> xr.DataArray:
        """Boolean mask: elements where investment is optional."""
        return ~self.mandatory

    @property
    def has_linked_periods(self) -> xr.DataArray:
        """Boolean mask: elements with linked_periods defined."""
        if self.linked_periods is None:
            return xr.DataArray(
                [False] * len(self.element_ids),
                dims=[self.dim_name],
                coords={self.dim_name: self.element_ids},
            )
        return self.linked_periods.notnull().any(dim=[d for d in self.linked_periods.dims if d != self.dim_name])
    ...
    @property
    def has_effects_of_investment_per_size(self) -> xr.DataArray:
        """Boolean mask: elements with any effects_of_investment_per_size defined."""
        if not self.effects_of_investment_per_size:
            return xr.DataArray(
                [False] * len(self.element_ids),
                dims=[self.dim_name],
                coords={self.dim_name: self.element_ids},
            )
        # Element has effect if any effect value is not NaN
        combined = xr.concat(list(self.effects_of_investment_per_size.values()), dim='effect')
        return combined.notnull().any(dim='effect')

    @property
    def has_effects_of_investment(self) -> xr.DataArray:
        """Boolean mask: elements with any effects_of_investment defined."""
        if not self.effects_of_investment:
            return xr.DataArray(
                [False] * len(self.element_ids),
                dims=[self.dim_name],
                coords={self.dim_name: self.element_ids},
            )
        combined = xr.concat(list(self.effects_of_investment.values()), dim='effect')
        return combined.notnull().any(dim='effect')

    @property
    def has_effects_of_retirement(self) -> xr.DataArray:
        """Boolean mask: elements with any effects_of_retirement defined."""
        if not self.effects_of_retirement:
            return xr.DataArray(
                [False] * len(self.element_ids),
                dims=[self.dim_name],
                coords={self.dim_name: self.element_ids},
            )
        combined = xr.concat(list(self.effects_of_retirement.values()), dim='effect')
        return combined.notnull().any(dim='effect')
